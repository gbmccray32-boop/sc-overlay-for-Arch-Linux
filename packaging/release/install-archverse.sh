...
VERSION='0.1.47-r31.alpha23.candidate16'
TAG='v0.1.47-r31.alpha23.candidate16-kde-test1'
...
Install the Candidate 16 KDE package for an Arch-, Fedora-, or Debian-family system.
...
if [[ "$tokens" =~ [[:space:]](arch|cachyos|endeavouros|manjaro)[[:space:]] ]]; then
  ...
  family='fedora'
  asset='archverse-overlay-0.1.47-1.r31.alpha23.candidate16.x86_64.rpm'
elif [[ "$tokens" =~ [[:space:]](debian|ubuntu|linuxmint|neon|pop)[[:space:]] ]]; then
  family='debian'
  asset='archverse-overlay_0.1.47~r31~alpha23~candidate16-1_amd64.deb'
else
...
if [[ "$(uname -m)" != 'x86_64' ]]; then
  printf 'Candidate 16 supports x86_64 only; detected %s.\n' "$(uname -m)" >&2
  exit 4
fi
if ! getconf GNU_LIBC_VERSION >/dev/null 2>&1; then
  printf 'Candidate 16 requires a glibc-based Linux distribution.\n' >&2
  exit 4
fi
if [[ "$supported" != 'yes' ]]; then
  printf 'Candidate 16 supports KDE X11 and KDE Wayland with XWayland only.\n' >&2
...
printf '\nArchVerse Candidate 16 is installed. Launch it from the application menu or run archverse-overlay.\n'
