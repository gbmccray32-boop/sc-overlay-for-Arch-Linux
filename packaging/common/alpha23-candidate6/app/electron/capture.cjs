...
async function captureLocationSyncCrop(finalPath, winRect) {
  ...
  try {
    // Normal/non-Gamescope launch: the existing capture contract already selects the bound game
    // display and falls through the Linux backends safely. Crop that display's own top-right.
