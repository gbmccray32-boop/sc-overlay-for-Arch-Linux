...
(async () => {
  ...
  const canvas = { getContext: () => ({ drawImage() {}, getImageData: () => ({ data: new Uint8ClampedArray(4) }) }), toBlob: callback => { pendingBlob = callback; } };
  const writes = [];
  const ipc = { on: (name, fn) => { handlers[name] = fn; }, send: (name, value) => sent.push({ name, value }) };
  vm.runInNewContext(source, {
    require: name => name === "./window-frame-encoder.cjs" ? { encodeWindowFrame: () => new Promise((resolve, reject) => { pendingBlob = value => value ? resolve(Buffer.from([1, 2, 3])) : reject(new Error("encoding failed")); }) } : name === "electron" ? { ipcRenderer: ipc } : name === "node:fs" ? {
  ...
  for (const stage of ["draw", "pngEncode", "pixelRead", "fileWrite", "total"]) assert(stage in frames[1].value.stages);
  ...
  console.log("Candidate 11 actual preload: repeated-frame retention, busy bound, full canvas, frame-stage metrics, encoding errors and track-ended reporting passed");
