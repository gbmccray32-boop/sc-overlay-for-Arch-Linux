...
// while the active renderer must decode the replacement PNG format.
const { app, BrowserWindow, ipcMain, nativeImage } = require("electron");
const assert = require("node:assert/strict");
const fs = require("node:fs");
const os = require("node:os");
const path = require("node:path");
const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "archverse-electron-fast-encoder-"));
...
fs.writeFileSync(preloadPath, `
  ...
  try {
    ...
    for (let pixel = 0; pixel < width * height; pixel++) {
      const at = pixel * 4;
      pixels[at] = (pixel * 73 + (pixel >>> 8)) % 256;
      pixels[at + 1] = (pixel * 127 + (pixel >>> 9)) % 256;
      pixels[at + 2] = (pixel * 199 + (pixel >>> 10)) % 256;
      pixels[at + 3] = 255;
    }
  ...
  ipcMain.once("archverse-encoder-test", (_event, result) => {
    try {
      ...
      for (const pixel of [0, 11, 100000, result.width * result.height - 1]) {
        const at = pixel * 4;
        assert.deepEqual([...bytes.subarray(at, at + 4)], [(pixel * 199 + (pixel >>> 10)) % 256, (pixel * 127 + (pixel >>> 9)) % 256, (pixel * 73 + (pixel >>> 8)) % 256, 255]);
      }
      console.log(`Candidate 12 real Electron preload lossless PNG: ${Math.round(result.elapsed)}ms, ${result.length} bytes, exact pixel samples`);
      finish();
