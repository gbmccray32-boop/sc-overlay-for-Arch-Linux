...
// Normal portal capture can take longer than a 700ms caller deadline under load. Keep the
// approved stream in use through bounded encoding spikes instead of starting Spectacle.
...
function createPersistentStarCitizenWindowCapture({
  ...
  function start(wantedSessionKey, gameWindowId) {
    ...
    stdoutBuffer = "";
    stderrTail = "";
    logger.log?.("[window-stream] waiting for Star Citizen window selection; screenshot fallbacks paused");
  ...
  async function capture({ gamePid, gameStartTicks, gameWindowId } = {}) {
    ...
    const result = new Promise((resolve, reject) => {
      ...
      hardTimer.unref?.();
      pending = { id, timer, hardTimer, resolve, reject };
    });
    try {
