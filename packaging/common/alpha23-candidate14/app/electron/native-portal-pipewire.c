...
static GVariant *lookup_typed_result(GVariant *results, const gchar *method,
	const gchar *key, const GVariantType *expected_type, GError **error)
{
	GVariant *value = g_variant_lookup_value(results, key, NULL);
	if (!value) {
		g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
			"%s returned no %s (expected D-Bus type %s)", method, key,
			g_variant_type_peek_string(expected_type));
		return NULL;
	}
	if (!g_variant_is_of_type(value, expected_type)) {
		g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
			"%s returned %s with D-Bus type %s; expected %s", method, key,
...
static gchar *extract_session_handle(GVariant *results, GError **error)
{
	/*
	 * ScreenCast.CreateSession specifies session_handle as a D-Bus string for
	 * historical compatibility, even though its contents are an object path.
	 * Accept an object-path-typed reply too, but validate either representation.
	 */
	g_autoptr(GVariant) value = g_variant_lookup_value(results,
		"session_handle", NULL);
	if (!value) {
		g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
			"CreateSession returned no session_handle (expected D-Bus type s)");
		return NULL;
	}
	if (!g_variant_is_of_type(value, G_VARIANT_TYPE_STRING) &&
	    !g_variant_is_of_type(value, G_VARIANT_TYPE_OBJECT_PATH)) {
		g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
			"CreateSession returned session_handle with D-Bus type %s; "
			"expected s or o", g_variant_get_type_string(value));
		return NULL;
	}
	const gchar *handle = g_variant_get_string(value, NULL);
	if (!g_variant_is_object_path(handle)) {
		g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
			"CreateSession returned session_handle with D-Bus type %s but "
			"invalid object-path contents", g_variant_get_type_string(value));
		return NULL;
	}
...
static gboolean create_portal_session(GError **error)
{
	...
	if (!create_results)
		return FALSE;
	session_handle = extract_session_handle(create_results, error);
	if (!session_handle)
		return FALSE;
	...
	g_autoptr(GVariant) streams = lookup_typed_result(start_results, "Start",
		"streams", G_VARIANT_TYPE("a(ua{sv})"), error);
	if (!streams)
		return FALSE;
...
static gboolean self_test_session_handle_value(GVariant *value,
	gboolean expected_success, const gchar *label)
{
	GVariantBuilder results_builder;
	g_variant_builder_init(&results_builder, G_VARIANT_TYPE_VARDICT);
	if (value)
		g_variant_builder_add(&results_builder, "{sv}", "session_handle", value);
	g_autoptr(GVariant) results = g_variant_ref_sink(
		g_variant_builder_end(&results_builder));
	g_autoptr(GError) error = NULL;
	g_autofree gchar *handle = extract_session_handle(results, &error);
	if (expected_success && (!handle || error)) {
		g_printerr("session_handle self-test failed for %s: %s\n", label,
			error ? error->message : "no handle returned");
		return FALSE;
	}
	if (!expected_success && (handle || !error)) {
		g_printerr("session_handle self-test accepted invalid %s reply\n", label);
		return FALSE;
	}
	return TRUE;
}

static gboolean self_test_portal_results(void)
{
	const gchar *handle =
		"/org/freedesktop/portal/desktop/session/archverse/test";
	return self_test_session_handle_value(g_variant_new_string(handle), TRUE,
		"spec string") &&
		self_test_session_handle_value(g_variant_new_object_path(handle), TRUE,
		"object path") &&
		self_test_session_handle_value(g_variant_new_uint32(7), FALSE,
		"wrong type") &&
		self_test_session_handle_value(g_variant_new_string("not/an/object/path"),
		FALSE, "invalid path") &&
		self_test_session_handle_value(NULL, FALSE, "missing value");
}

int main(int argc, char **argv)
{
	if (argc == 2 && g_str_equal(argv[1], "--self-test")) {
		if (!self_test_portal_results())
			return 9;
		gst_init(&argc, &argv);
