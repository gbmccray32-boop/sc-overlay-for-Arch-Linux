...
class LinuxFocusController {
  ...
  windowClassName(id) {
    if (this.platform !== "linux" || !/^\d+$/.test(String(id || ""))) return "";
    try {
      // Fedora 44's xdotool can abort in xdo_get_window_classname for a valid XWayland window.
      // xprop reads the same WM_CLASS property without passing through that libxdo code path.
      return String(this.commandRunner("xprop", ["-id", String(id), "WM_CLASS"], {
  ...
  activeWindowDetails() {
    ...
    try {
      ...
      const className = this.windowClassName(id);
  ...
  detailsForWindowId(id) {
    ...
    try {
      ...
      const className = this.windowClassName(windowId);
