  ...
  function spawnWorker() {
    ...
    child.on("message", (message) => {
      ...
      if (message.type === "result") {
        ...
        const reason = new Error(String(message.error || "RapidOCR worker request failed"));
        request.reject(reason);
        // A native detector exception can leave OpenCV/ONNX state corrupt while the Node child
        // remains alive. Reusing that process produced thousands of identical fallback failures
        // during a long field session. Discard it so the next fresh frame gets a clean runtime.
        if (restartOnFailure && !closing && worker === record) {
          record.recycleRequested = true;
          detach(record);
          restarts += 1;
          logger.warn?.(`[ocr-worker] request failed; recycling worker for the next fresh frame: ${reason.message}`);
    ...
    child.once("exit", (code, signal) => {
      ...
      rejectGeneration(record, reason);
      if (!closing && !record.timedOut && !record.recycleRequested) {
        if (restartOnFailure) {
          restarts += 1;
