...
const { decodePortalFrame } = require("./portal-raw-frame.cjs"); // ARCHVERSE_ALPHA23_NATIVE_PORTAL_PIPEWIRE
...
async function captureWithPersistentStarCitizenWindow(disp) {
  ...
  const streamInfo = starCitizenWindow.sourceInfo?.();
  const sourceCrop = streamInfo?.engine === "native-gstreamer"
    ? normalCanvasCropForSize({ width: streamInfo.width, height: streamInfo.height }, disp)
    : null;
  ...
  let full;
  try { full = decodePortalFrame(got, nativeImage); }
  catch (error) { starCitizenWindow.stop?.("invalid frame transport"); throw new Error(`could not decode isolated Star Citizen window frame: ${error.message}`); }
  if (!full || full.isEmpty()) {
    starCitizenWindow.stop?.("empty frame transport");
    throw new Error("isolated Star Citizen window frame decoded empty");
  }
  const sourceSize = Number(got.sourceWidth) > 0 && Number(got.sourceHeight) > 0
    ? { width: Number(got.sourceWidth), height: Number(got.sourceHeight) }
    : full.getSize();
  const nativeCropApplied = sourceCrop && got.nativeCrop &&
    ["x", "y", "width", "height"].every(key => Number(got.nativeCrop[key]) === Number(sourceCrop[key]));
  const rawImage = nativeCropApplied ? full : cropFullCanvasToDisplay(full, disp);
...
function cropFullCanvasToDisplay(full, disp) {
  const size = full.getSize();
  const crop = normalCanvasCropForSize(size, disp);
  return crop ? full.crop(crop) : full;
}

function normalCanvasCropForSize(size, disp) {
  const displays = screen.getAllDisplays();
...
async function captureGame(winRect) {
  ...
  for (const name of order) {
    try {
      ...
      // A live KDE chooser is not a failed capture. Running Spectacle here opens a second
      // chooser over the first one and can make the selected window ambiguous.
      if (!hasGamescope && e?.code === "PORTAL_SELECTION_PENDING") throw e;
...
async function captureLocationWithPortalWindow(finalPath) {
  ...
  const full = decodePortalFrame(got, nativeImage);
...
async function captureLocationSyncCrop(finalPath, winRect) {
  ...
  if (!session?.gamescopePid) {
    ...
    try { return await captureLocationWithPortalWindow(finalPath); }
      ...
      if (error?.code === "PORTAL_SELECTION_PENDING") throw error;
      errors.push(`portal-window: ${error?.message || error}`);
    }
...
function startFabCapture({ port, configDir, onStatus, onSidecarTransportFailure, onSidecarTransportSuccess, devTools = false }) {
  ...
  let lastRefineryDiagnosticAt = 0;
  let lastRefineryDiagnosticKey = "";
  const REFINERY_ACTIVE_VEHICLE_PROBE_MS = 15000;
  const REFINERY_SIGNATURE_QUIET_MS = 12000;
    ...
    try {
      ...
      if (process.platform === "linux") {
        ...
        const readRegion = async (key, enabled, everyMs = POLL_MS, alreadyDue = false, propagateFailure = false) => {
          ...
          try {
            ...
            if (key === "refinery") {
              const jobs = Array.isArray(read?.jobs) ? read.jobs.length : 0;
              const reason = String(read?.refineryDiagnostic?.reason || (read?.kind === "refinery" ? "accepted" : "unclassified"));
              const sample = got.lines.map((line) => String(line?.text || "").trim())
                .filter(Boolean).slice(0, 12).join(" | ").slice(0, 320).replace(/[\r\n]+/g, " ");
              const diagnosticKey = `${got.engine}:${read?.kind || "none"}:${jobs}:${reason}:${sample}`;
              const diagnosticNow = Date.now();
              if (diagnosticKey !== lastRefineryDiagnosticKey || diagnosticNow - lastRefineryDiagnosticAt >= 30000) {
                console.log(`[ocr-refinery] engine=${got.engine} region=${got.region.x},${got.region.y},${got.region.width}x${got.region.height} lines=${got.lines.length} result=${read?.kind || "none"} jobs=${jobs} reason=${reason} text="${sample}"`);
                lastRefineryDiagnosticAt = diagnosticNow;
                lastRefineryDiagnosticKey = diagnosticKey;
              }
            }
        ...
        runLinuxBackgroundOcr = (onlyKey = null) => {
          ...
          for (let step = 0; step < linuxBackgroundKeys.length; step += 1) {
            ...
            const everyMs = key === "refinery" && onlyKey === "refinery" ? REFINERY_ACTIVE_VEHICLE_PROBE_MS : POLL_MS;
            if (!enabled[key] || !linuxOcrDue(key, everyMs)) continue;
      ...
      if (mining && (process.platform !== "linux" || linuxMiningNumericDue)) {
        try {
          ...
          const confirmationWindowMs = /^(?:gamescope-pipewire|portal-pipewire-window|electron-x11-window-stream)$/.test(cap.method)
            ? 1600
      ...
      const linuxVehicleMiningActive = mining && process.platform === "linux" && vehiclePresence.active === true;
      // A stale Game.log ship channel must not permanently suppress a refinery terminal. After
      // the Resource Signature lane has been quiet for a bounded interval, permit one low-rate
      // refinery-only probe. Other auxiliary readers remain deferred, and an active signature
      // lock keeps the latency-critical Mining lane exclusive.
      const refineryProbeAllowed = linuxVehicleMiningActive
        && (!sigBoxAt || Date.now() - sigBoxAt >= REFINERY_SIGNATURE_QUIET_MS);
      if (runLinuxBackgroundOcr && !linuxVehicleMiningActive) {
        runLinuxBackgroundOcr();
        stage.backgroundOcr = linuxBackgroundInFlight.size ? "background-dispatched" : "background-idle";
      } else if (runLinuxBackgroundOcr && refineryProbeAllowed) {
        runLinuxBackgroundOcr("refinery");
        stage.backgroundOcr = linuxBackgroundInFlight.size ? "refinery-probe" : "refinery-wait";
      ...
      if (e?.code === "PORTAL_SELECTION_PENDING") {
        stage.selectionPending = true;
        return;
      }
