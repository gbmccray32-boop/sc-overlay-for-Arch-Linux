...
function createPersistentStarCitizenWindowCapture({
  ...
  function acceptMessage(message) {
    ...
    if (message.type === "ready") {
      ...
      logger.log?.(`[window-stream] portal PipeWire ready source="${message.sourceName || "Star Citizen"}" ${message.width}x${message.height}`);
      return;
    }
    if (message.type !== "frame" && message.type !== "error") return;
    if (message.type === "error" && Number(message.id) === 0) {
      initializationError = String(message.error || "portal PipeWire initialization failed");
      disabledSessionKey = sessionKey;
      restartAfter = Number.POSITIVE_INFINITY;
      logger.warn?.(`[window-stream] portal PipeWire initialization failed: ${initializationError}; disabled for this Star Citizen session`);
      return;
    }
  ...
  function start(wantedSessionKey, gameWindowId) {
    ...
    proc.once("exit", (code, signal) => {
      ...
      child = null;
      ready = null;
      if (disabledSessionKey !== sessionKey) restartAfter = now() + START_RETRY_MS;
  ...
  async function capture({ gamePid, gameStartTicks, gameWindowId } = {}) {
    const targetWindowId = /^\d+$/.test(String(gameWindowId || "")) ? String(gameWindowId) : "";
    const wantedSessionKey = `${Number(gamePid) || 0}:${String(gameStartTicks || "")}:${targetWindowId}`;
