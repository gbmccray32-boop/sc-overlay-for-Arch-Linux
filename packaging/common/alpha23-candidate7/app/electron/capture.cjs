...
async function captureFullX11Window() {
  ...
  try { fs.unlinkSync(x11WindowCapturePath); } catch {}
  const env = { ...process.env, GST_XINITTHREADS: "1" };
  if (process.env.SC_TRACKER_HOST_DISPLAY) env.DISPLAY = process.env.SC_TRACKER_HOST_DISPLAY;
  await runFile("gst-launch-1.0", [
    "-q", "-e", "ximagesrc", `xid=${boundGameWindowId}`, "show-pointer=false", "use-damage=false", "num-buffers=1", "!",
    "videoconvert", "!", "pngenc", "compression-level=1", "!", "filesink", `location=${x11WindowCapturePath}`,
  ], { timeout: 2000, maxBuffer: 2 * 1024 * 1024, env });
  const image = nativeImage.createFromPath(x11WindowCapturePath);
  if (!image || image.isEmpty()) throw new Error("exact-XID X11 capture decoded empty");
  return image;
}
