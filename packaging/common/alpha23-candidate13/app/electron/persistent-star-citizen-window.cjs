...
// A native helper owns the KDE portal session and its restricted PipeWire remote on Wayland.
// Pure X11 retains the isolated Electron window stream. Neither helper may freeze the overlay.
...
// The native helper owns a shorter internal frame wait. Retain Candidate 12's 1400ms parent
// deadline so the helper can report an unavailable frame before the caller falls back.
const REQUEST_TIMEOUT_MS = 1400;
const STALLED_FRAME_TIMEOUT_MS = 8000;
...
function createPersistentStarCitizenWindowCapture({
  ...
  nativePortalPath = path.join(__dirname, "archverse-portal-pipewire"),
  platform = process.platform,
  environment = process.env,
  requestTimeoutMs = REQUEST_TIMEOUT_MS,
  stalledFrameTimeoutMs = STALLED_FRAME_TIMEOUT_MS,
} = {}) {
  ...
  let lastNativeDiagnosticAt = -Infinity;
  let closed = false;
  let stdoutBuffer = "";
  let stderrTail = "";
  ...
  function acceptMessage(message) {
    ...
    if (message.type === "ready") {
      ready = message;
      initializationError = "";
      logger.log?.(`[window-stream] ${message.transport || "window"} ready engine=${message.engine || "electron"} source="${message.sourceName || "Star Citizen"}" ${message.width}x${message.height}`);
      return;
    }
    ...
    if (Number(message.sourceWidth) > 0 && Number(message.sourceHeight) > 0 && ready) {
      ready.width = Number(message.sourceWidth);
      ready.height = Number(message.sourceHeight);
    }
    message.cropKey = pending.cropKey;
    if (message.stages && now() - lastNativeDiagnosticAt >= 10000) {
      lastNativeDiagnosticAt = now();
      logger.log?.(`[window-stream] native frame stages ${JSON.stringify(message.stages).slice(0, 400)}`);
    }
  ...
  function start(wantedSessionKey, gameWindowId) {
    ...
    const env = { ...environment, ARCHVERSE_CAPTURE_HELPER: "1" };
    delete env.ELECTRON_RUN_AS_NODE;
    const targetWindowId = /^\d+$/.test(String(gameWindowId || "")) ? String(gameWindowId) : "";
    const hostWaylandDisplay = String(env.SC_TRACKER_HOST_WAYLAND_DISPLAY || env.WAYLAND_DISPLAY || "");
    const useNativePortal = platform === "linux" && !!hostWaylandDisplay && fs.existsSync(nativePortalPath);
    const configDirectory = String(env.SC_TRACKER_CONFIG_DIR || "").trim();
    const restoreTokenPath = path.join(configDirectory ? path.resolve(configDirectory) : outputDirectory,
      "portal-capture-restore-token");
    fs.mkdirSync(path.dirname(restoreTokenPath), { recursive: true, mode: 0o700 });
    const command = useNativePortal ? nativePortalPath : electronPath;
    const args = useNativePortal
      ? [outputDirectory, restoreTokenPath]
      : [helperPath, outputDirectory, targetWindowId];
    const proc = spawner(command, args, {
      stdio: ["pipe", "pipe", "pipe"],
      windowsHide: true,
      env,
    });
    proc.nativePortal = useNativePortal;
    ...
    stdoutBuffer = "";
    stderrTail = "";
    logger.log?.(`[window-stream] waiting for Star Citizen window selection via ${useNativePortal ? "native portal PipeWire" : "isolated Electron"}; screenshot fallbacks paused`);
  ...
  async function capture({ gamePid, gameStartTicks, gameWindowId, sourceCrop = null } = {}) {
    const targetWindowId = /^\d+$/.test(String(gameWindowId || "")) ? String(gameWindowId) : "";
    ...
    if (!ready) {
      const error = new Error("Star Citizen portal PipeWire stream is warming while waiting for window selection");
      error.code = "PORTAL_SELECTION_PENDING";
      throw error;
    }
    const crop = sourceCrop && [sourceCrop.x, sourceCrop.y, sourceCrop.width, sourceCrop.height]
      .every(Number.isInteger) && sourceCrop.x >= 0 && sourceCrop.y >= 0 && sourceCrop.width > 0 && sourceCrop.height > 0
      ? { x: sourceCrop.x, y: sourceCrop.y, width: sourceCrop.width, height: sourceCrop.height }
      : null;
    ...
    if (latestFrame) {
      ...
      if (cached.cropKey === cropKey && age >= 0 && age <= 1500 && Number(cached.sequence) > deliveredSequence) {
    ...
    const result = new Promise((resolve, reject) => {
      ...
      const hardTimer = setTimeout(() => {
        if (!pending || pending.id !== id) return;
        // A renderer that dies inside native encoding can leave its Electron parent alive.
        // Keep ordinary late frames, but stop this exact game session after a real stall.
        disableSession(`frame ${id} stalled for ${stalledFrameTimeoutMs}ms after portal approval`);
        stop("stalled frame");
      }, stalledFrameTimeoutMs);
      hardTimer.unref?.();
      pending = { id, timer, hardTimer, resolve, reject, cropKey };
    });
    try {
      child.stdin.write(`${JSON.stringify({ type: "capture", id, ...(crop ? { crop } : {}) })}\n`);
    } catch (error) {
      settlePending(error);
    }
  ...
  function sourceInfo() {
    return ready ? { width: Number(ready.width), height: Number(ready.height),
      transport: ready.transport, engine: ready.engine } : null;
  }

  return { capture, close, stop, sourceInfo };
}

module.exports = { createPersistentStarCitizenWindowCapture, __test: { REQUEST_TIMEOUT_MS, START_RETRY_MS, STALLED_FRAME_TIMEOUT_MS } };
