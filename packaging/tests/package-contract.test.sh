...
rpm_package="$3"
deb_package="$4"
expected_payload_sha='1c5bb72be177704ff1dc18788d9d65457e6d9965ddf68b7c8bdee6201c66495f'
payload_name='ArchVerse-Native-0.1.47-r31.alpha23.candidate18'
...
for family in arch rpm deb; do
  ...
  [[ -f "$root/usr/lib/udev/rules.d/70-archverse-input.rules" ]] || {
    printf '%s active-session keyboard rule missing\n' "$family" >&2
    exit 4
  }
  ...
  cmp "$work/payload.files" "$work/$family.files" >/dev/null || {
    printf '%s package changed Candidate 18 payload file bytes\n' "$family" >&2
    diff -u "$work/payload.files" "$work/$family.files" | head -n 80 >&2 || true
    exit 5
  }
  cmp "$work/payload.links" "$work/$family.links" >/dev/null || {
    printf '%s package changed Candidate 18 payload symlinks\n' "$family" >&2
    exit 5
  }
  (
    cd "$app_root"
  ...
  printf 'PASS: %s package preserves all Candidate 18 payload files and links\n' "$family"
done

grep -q '^pkgname = archverse-overlay$' "$work/arch/.PKGINFO"
grep -q '^pkgver = 0.1.47.r31.alpha23.candidate18-1$' "$work/arch/.PKGINFO"
[[ "$(dpkg-deb -f "$deb_package" Package)" == 'archverse-overlay' ]]
[[ "$(dpkg-deb -f "$deb_package" Version)" == '0.1.47~r31~alpha23~candidate18-1' ]]
